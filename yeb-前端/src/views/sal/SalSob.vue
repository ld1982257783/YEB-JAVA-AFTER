      ...
      <el-table :data="slaries" border stripe>
        ...
        <el-table-column label="操作">
          ...
          <template slot-scope="scope">
            <el-button type="primary" @click="showEditSalry(scope.row)"
              >编辑</el-button
            >
...
export default {
  ...
  data() {
    //这里存放数据
    return {
      slaries: [], //el-table关联数组  发送后端请求获得
      dialogVisible: false, //控制 是否 展示dialog弹出框
      dialogTitle: "添加工资账套", //dialog标题
      activeItemIndex: 0, //默认激活在哪里
      salaryItemName: [
        "账套名称",
        "基本工资",
        "交通补助",
  ...
  methods: {
    ...
    deleteSalary(data) {
      this.$confirm(
        ...
        .then(() => {
          deleteRequest("/salary/sob/" + data.id).then(
            (resp) => {
              if (resp) {
                this.initSlaries();
              }
    ...
    initSlaries() {
      this.getRequest("/salary/sob/").then(
        (resp) => {
          if (resp) {
            this.slaries = resp;
          }
